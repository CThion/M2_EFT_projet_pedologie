...
mosaicplot(Auger$mica ~ Auger$Silt_class, data = Auger,
           color = TRUE, main = "Mica-Silt relation", xlab="Mica content", ylab="Silt content")

mosaicplot(Auger$Silt_class ~ Auger$mica, data = Auger,
           color = TRUE, main = "Mica Silt relation", xlab="Silt content", ylab="Mica content")
#ou soit
...
```
```{r}
# Your original mosaic plot
mosaicplot(Auger$mica ~ Auger$Silt_class, data = Auger,
           color = TRUE, main = "Mica Silt relation", xlab = "Mica content", ylab = "Silt content")

# Define categories and corresponding colors
categories <- unique(Auger$Silt_class)  # Assuming Silt_class contains categories
colors <- c("white", "#808080", "#404040")  # Define colors for each category (grey shades)

# Create a legend manually using the legend function, adjust coordinates
legend("topright", legend = categories, fill = colors, x.intersp = 0.5, y.intersp = 0.5, xjust = 1, yjust = 1)
...
# Assuming Auger is your dataset
# Using dplyr to create a summary of counts
plot_data <- Auger %>%
  count(Silt_class, mica) %>%
  mutate(mica = factor(mica), Silt_class = factor(Silt_class))

# Create the plot with switched axes
ggplot(plot_data, aes(x = mica, y = Silt_class, fill = n)) +
  geom_tile() +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Mica Silt relation", x = "Mica content", y = "Silt content")
