...
AGB_contour <- Merged_contour %>%
  ...
  summarise(
    amount_trees = n(),
    sp_richness = n_distinct(full_species),
    trees_per_area = amount_trees / total_area,
    sp_richness_area = sp_richness / total_area,
...
in pairwise no significant effect anywhere, but there is in main total test -> explain this how?
ask question about DBH

shapiro test: 0.9472 p value => normally distributed
but not homogeneous variances
=> Kruskal wallis

```{r poisson model for amount of trees}

install.packages("betareg")
library(betareg)
M1 <- betareg(trees_per_area ~ HSD_class, data = AGB_contour)
