...
#plot to show tree density per HSD class
plot(AGB_contour$HSD_class, AGB_contour$tree_density)

ggplot(AGB_contour, aes(x = HSD_class, y = tree_density)) +
  geom_point() +
  labs(x = "HSD Class", y = "Tree density", title = "Number of trees in function of different HSD Classes")

#plot with ha instead of square meters
ggplot(AGB_contour, aes(x = HSD_class, y = tree_density_ha)) +
  geom_point() +
  labs(x = "HSD Class", y = "Tree density per hectare", title = "Number of trees in function of different HSD Classes")
```
...
#plot to show species richness per HSD class
plot(AGB_contour$HSD_class, AGB_contour$sp_richness_density)

ggplot(AGB_contour, aes(x = HSD_class, y = sp_richness_density)) +
  geom_point() +
  labs(x = "HSD Class", y = "Species richness (proportioned by area)", title = "Species richness in function of different HSD Classes") +
  scale_y_continuous(breaks = seq(min(AGB_contour$sp_richness_density), max(AGB_contour$sp_richness_density), by = 0.03), labels = function(x) sprintf("%.3f", x))

#sp richness per ha
ggplot(AGB_contour, aes(x = HSD_class, y = sp_richness_ha)) +
  geom_point() +
  labs(x = "HSD Class", y = "Species richness in ha", title = "Species richness in function of different HSD Classes") +
  scale_y_continuous(breaks = seq(min(0), max(AGB_contour$sp_richness_ha), by = 200))
```
40to50 HSD has much higher species richness per area, is this linked to a specific topography? why would this be?

```{r visual mean DBH}
#DBH in function of different HSD classes
ggplot(Merged_contour, aes(x = HSD_class, y = DBH)) + 
  geom_boxplot() + 
  labs(x = "HSD Class", y = "DBH", title = "DBH in function of different HSD Classes")

#mean DBH (one value) per class
...
#non normal data
#maybe make bigger groups of classes
#create a continuous variable of HSD
...
ggplot(data = filtered_DBH, aes(HSD_class, DBH)) +
  geom_boxplot() +
  geom_point() +
  labs(x = "HSD Class", y = "DBH (max 10)", title = "Filtered DBH in function of different HSD Classes")

#still no difference visible in mean, again outliers that play a big role
