...
```{r loop to sum areas per dataframe to get total area}
# List of Area dataframes
area_dfs <- list(Area_1, Area_2, Area_3, Area_4, Area_5, Area_6, Area_7)

# Function to add total_area column to each dataframe
add_total_area_column <- function(df) {
  total_area <- sum(df$Area)  # Calculate the sum of 'Area' column within the dataframe
  df$total_area <- total_area  # Add a new column with the total area for this dataframe
  return(df)
}

# Apply the function to each dataframe in the list
area_dfs_with_total_area <- lapply(area_dfs, add_total_area_column)

# Assign the updated dataframes back to their respective variables
Area_1 <- area_dfs_with_total_area[[1]]
Area_2 <- area_dfs_with_total_area[[2]]
Area_3 <- area_dfs_with_total_area[[3]]
Area_4 <- area_dfs_with_total_area[[4]]
Area_5 <- area_dfs_with_total_area[[5]]
Area_6 <- area_dfs_with_total_area[[6]]
Area_7 <- area_dfs_with_total_area[[7]]


#create one area dataframe
Area <- rbind(Area_1, Area_2, Area_3, Area_4, Area_5, Area_6, Area_7) %>%
  distinct(HSD_class, .keep_all = TRUE) %>%
  select(c("HSD_class", "total_area"))
...
AGB_contour <- Merged_contour %>%
  ...
  mutate(
    ...
    mean_DBH = mean(DBH, na.rm = TRUE)  # Calculate mean DBH within each HSD_class
  ) %>%
  summarise(
    amount_trees = n(),
    sp_richness = n_distinct(full_species),
    trees_per_area = sum(amount_trees) / total_area,
    mean_DBH = mean(mean_DBH, na.rm = TRUE)  # Calculate the mean of mean_DBH per HSD_class
  ) %>%
  distinct(HSD_class, .keep_all = TRUE)
...
```{r}
shapiro.test(AGB_contour$mean_DBH)   #test normality

ggplot(Merged_contour, aes(x = HSD_class, y = DBH)) + geom_boxplot()  #boxplot looks strange

leveneTest(aov(Merged_contour$DBH ~ Merged_contour$HSD_class))  #test homoscedasticity


kruskal.test(Merged_contour$DBH ~ Merged_contour$HSD_class)

pairwise.wilcox.test(Merged_contour$DBH, Merged_contour$HSD_class, p.adjust.method="bonferroni")

dunnTest(Merged_contour$DBH, Merged_contour$HSD_class, method = "bonferroni")  #normally good method for unbalanced designs (we have more DBH measurements for certain classes!)

```

shapiro test: 0.9472 p value => normally distributed
but not homogeneous variances
=> 
