...
dataPitP1$pit <- rep("P1", 100); dataPitP2$pit <- rep("P2", 100); dataPitP3$pit <- rep("P3", 100); dataPitP4$pit <- rep("P4", 100) #add a the variable pit to distinguish each sheet data before merging them
...
# dataPit$texture <- factor(dataPit$texture, levels = c("A", "AS", "SA", "SL", "LSA", "LAS"))
...
#----make a loop for agregate all layer with identical texture----
d=1 #counter id
...
#meaning micas by thickness/10, which is equivalent to the number of layers in one given horizon
dataPitHorizon$micas <- round(dataPitHorizon$micas / (dataPitHorizon$thickness/10))

#define all micas content value >3 as 3 (i.e as high micas content)
...
For the calculation of root Index, we weight each root count by the
diameter of the root class. The diameter classes are in millimeter: d1:
$<2$, d2: $2-5$, d3: $5-10$ and d4: $≥ 10$.

```{r}
# root Index by horizon
dataPitHorizon <- dataPitHorizon %>% mutate(rootIndex=round((rd1+rd2*2+rd3*5+rd4*10), 2)) #add rootIndex variable

# root Index by layer
dataPitLayer <- dataPitLayer %>% mutate(rootIndex=(rd1+rd2*2+rd3*5+rd4*10))
```
...
P2all <- sum(dataPitLayer$rootIndex[which(dataPitLayer$pit=="P2")])
P2inter <- sum(dataPitLayer$rootIndex[which(dataPitLayer$pit=="P2" & dataPitLayer$siltcontent=="intermediate")])
P2high <- sum(dataPitLayer$rootIndex[which(dataPitLayer$pit=="P2" & dataPitLayer$siltcontent=="high")])
P2inter/P2all
P2high/P2all
P2high/P2inter
...
```{r}
P2top <- sum(dataPitLayer$rootIndex[which(dataPitLayer$pit=="P2" & between(dataPitLayer$depth, 0, 50))])
P2foc <- sum(dataPitLayer$rootIndex[which(dataPitLayer$pit=="P2" & between(dataPitLayer$depth, 50, 70))])
