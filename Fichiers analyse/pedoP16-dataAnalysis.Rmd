...
```{r}
dataPit$depth <- as.numeric(dataPit$depth)
dataPitHorizon <- dataPit %>%
  # filter(pit=="F1") %>%
  select(c(pit, depth, texture, rd1)) %>%
  group_by(pit, depth) %>% 
  summarise(
    texture=unique(texture),
    rd1=mean(rd1))
ungroup(dataPitHorizon)
...
while(!is.na(dataPitHorizon$texture[d+1])){#while it remains layer to analyse
  ...
  if(up==down){#if the next layer has the same texture
    ...
    dataPitHorizon$rd1[d] <- mean(dataPitHorizon$rd1[d], dataPitHorizon$rd1[d+1]) 
