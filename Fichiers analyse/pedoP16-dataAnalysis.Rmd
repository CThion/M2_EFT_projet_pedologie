...
dataPitHorizon <- dataPitHorizon %>% mutate(rootDensity=round((rd1*2+rd2*5+rd3*10+rd4*20)/thickness, 2))

# root biomass by layer
dataPitLayer <- dataPitLayer %>% mutate(rootDensity=(rd1*2+rd2*5+rd3*10+rd4*20)/10)
```
...
ggplot(dataPitHorizon, aes(x = pit, y = thickness, fill = texture, label = rootDensity)) +
